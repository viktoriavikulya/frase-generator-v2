...
The engine scripts are injected dynamically with `async = false`: execution order is guaranteed,
but loading is not parser-blocking, so the watermark/logo `onload` handlers in `js/config.js`
could fire before `js/app.js` defined `draw()` (a preexisting `ReferenceError: draw is not
defined` in the console). Those handlers now guard the early repaint with
`typeof draw === "function"` — the `assetsReady` flags are set before the call and `js/app.js`
redraws on load and after `fonts.ready`, so skipping that repaint changes nothing visually.
