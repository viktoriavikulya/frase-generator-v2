...
- `panel.html` is the **only main HTML page** and the GitHub Pages entry point, with five tabs:
  `Publicar Ahora` (publish form), `Curar Frases`, `Agregar Frases` and `Armar Carruseles` (the
  native Archivo X UI, split into raw intake, curation and carousel-assembly views), and `Preview`
  (a standalone render tester for any text/mode/color). The official entry URL is `/panel.html` —
