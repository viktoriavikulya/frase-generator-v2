...
- Ver historial de ejecuciones: `Operaciones` -> `Actualizar historial` (hasta 10 runs recientes mezclados de `publish.yml` y `metrics.yml`; incluye `repository_dispatch`, `schedule` e historicos porque no filtra por event ni branch). Si la consulta de uno de los dos workflows falla, el panel muestra igual los runs del otro con una advertencia; los errores de la API de GitHub (401/403/404) se reportan con mensajes claros.
