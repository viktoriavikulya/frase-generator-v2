...
| `npm run curate:archivo-x` | `node scripts/dev/archive-curator-server.js` | curaduría | Medio | Para levantar la API de Archivo X en local (`http://localhost:5177`) mientras se usa `panel.html`. | Usa credenciales de Google Sheets (`SHEET_ID`, service account). Escribe en la pestaña `archivo_x`. Visitar la URL directo en el navegador redirige a `panel.html#curate` — la UI legacy que corría ahí (`tools/archivo-x-curator.html`) se eliminó del repo en la Fase C5. |
...
| `npm run render` | `node scripts/dev/render-preview.js` | dev/debug | Bajo | Preview rápido de una frase suelta, resultado en `/output`. | No toca el Sheet. Sirve también como verificación del motor tras tocar `js/` (los `onload` de assets en `js/config.js` llevan un guard antes de llamar `draw()` — no quitarlo). |
