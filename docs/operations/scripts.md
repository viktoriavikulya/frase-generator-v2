...
| `npm run doctor` | `node scripts/dev/doctor.js` | doctor/validación | Bajo | Después de tocar pipeline, docs o paletas — valida archivos, sintaxis, exports y docs. | Solo lectura local, no llama al Sheet real. Desde la Fase A.11 valida la existencia de los 18 scripts de `package.json` (antes 14) y la sintaxis de 22 archivos `.js` (antes 16) — incluye a los scripts de `scripts/dev/` que antes no se autochequeaban, más `fetch-metrics.js`. Sigue sin cubrir `scripts/pipeline/{run-once,run-single,run-carousel,unlock-row}.js`, `scripts/libs/{facebook,instagram,graph-client,render,telegram}-lib.js`, `scripts/core/*` ni `scripts/auth/*` — candidato para una fase futura. |
