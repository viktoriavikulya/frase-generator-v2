...
| `tools/archivo-x-curator.html` | UI legacy de curaduría ("Curaduría" + "Publicar carruseles"), **conservada por compatibilidad/historial**. Ya no es el fallback visible: desde la Fase C3, `archive-curator-server.js` redirige (302) tanto la raíz como `/archivo-x-curator.html` hacia `panel.html#curate` en vez de servir este archivo. | `CLAUDE.md`, `docs/architecture/entrypoints.md`, `docs/architecture/mapa-del-proyecto.md`, `docs/operations/scripts.md`, `doctor.js` (`REQUIRED_FILES`), `archive-curator-server.js` (ya no lo sirve, pero sigue en `tools/` sin borrarse), `panel.html` (URL por defecto del curador apunta al mismo backend), `render.yaml` (nombre del servicio). | UI legacy conservada | Medio — ya no está en la ruta caliente de producción (nadie la ve por defecto), pero sigue siendo el archivo que un revert del redirect volvería a servir. No es HTML muerto. | No mover ni borrar en esta fase — ver `docs/architecture/entrypoints.md`. |
...
- **curaduría activa / UI legacy conservada:** `archive-curator-server.js` (activo, es la API real) y `tools/archivo-x-curator.html` (legacy, ya no es el fallback visible desde la Fase C3) — los dos archivos "delicados" de esta carpeta, ya documentados aparte.
