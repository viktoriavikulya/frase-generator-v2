...
Inventario detallado de cada archivo en `scripts/dev/` y `tools/` — qué hace, dónde está
referenciado, y qué tan seguro es tocarlo/moverlo más adelante. Complementa a
...
| --- | --- | --- | --- | --- | --- |
| `scripts/dev/archive-curator-server.js` | Servidor Express: API de Archivo X (`/api/phrases`, `/api/raw-phrases`, `/api/plan-carruseles`, `/api/taxonomy`) + fallback que sirve `tools/archivo-x-curator.html`. | `package.json` (`curate:archivo-x`, `start:archivo-x`), `render.yaml` (`startCommand` de producción), `doctor.js` (`REQUIRED_FILES` + `CHECKED_JS_FILES`), `CLAUDE.md`, `docs/architecture/entrypoints.md`, `docs/architecture/mapa-del-proyecto.md`, `docs/operations/scripts.md`, `README.md`. | curaduría activa / fallback activo | **Alto** — es el único script de `scripts/dev/` que `doctor.js` verifica explícitamente (existencia + sintaxis), y es literalmente el `startCommand` del deploy de Render. Moverlo rompe producción si no se actualiza `render.yaml`, `package.json` y el propio `path.join(...)` interno hacia `tools/archivo-x-curator.html`. | No mover en esta fase. Ver `docs/architecture/entrypoints.md`. |
| `scripts/dev/doctor.js` | Valida archivos requeridos, sintaxis, exports, docs y sync de paletas. Es `npm run doctor`. | `package.json` (`doctor`), `README.md`, `CLAUDE.md`, `docs/operations/scripts.md`, `.claude/settings.local.json` (config local no versionada). | dev activo / diagnóstico | Medio — **doctor.js no se valida a sí mismo** (no está en su propia `REQUIRED_FILES` ni `CHECKED_JS_FILES`). Si se mueve, `npm run doctor` falla con "cannot find module" recién al ejecutarlo, no antes. | No mover. Es la herramienta de validación central del repo. |
| `scripts/dev/doctor-sheet.js` | Audita columnas/estados del Google Sheet real — **solo lectura**, no hay ninguna llamada de escritura en el archivo. Es `npm run doctor:sheet`. | `package.json` (`doctor:sheet`), `CLAUDE.md`, `docs/operations/scripts.md`. | diagnóstico | Medio — no está en `REQUIRED_FILES` de `doctor.js`, así que moverlo no se detecta hasta correr `npm run doctor:sheet` y que falle. | No mover sin actualizar `package.json` en el mismo cambio. |
| `scripts/dev/check-palettes-sync.js` | Compara `scripts/config/retro-palettes.js` contra `js/palettes.js`; falla con `exit(1)` si difieren (usable en CI). Es `npm run check-palettes-sync`. | `package.json`, `CLAUDE.md`, `README.md`, `docs/architecture/mapa-del-proyecto.md`, `docs/operations/scripts.md`. | paletas / diagnóstico | Medio — mismo caso: no está en `REQUIRED_FILES` de `doctor.js`. | No mover. Nota menor: su propio comentario interno dice `npm run check-palettes-sync ← agregar a package.json si querés`, pero ese script **ya está** en `package.json` — es un comentario desactualizado dentro del archivo, no un problema funcional. No lo corregí en esta fase (no estaba en el alcance permitido). |
| `scripts/dev/sync-palettes.js` | Sincroniza `scripts/config/retro-palettes.js` → `js/palettes.js` (reemplaza el bloque entre los marcadores `// RETRO_PALETTES_START/END`). Es `npm run sync-palettes`. | `package.json`, `CLAUDE.md`, `README.md`, `docs/architecture/mapa-del-proyecto.md`, `docs/operations/scripts.md`, y usado internamente por `check-palettes-sync.js` (menciona el comando en su mensaje de error). | paletas | Medio — mismo caso, no está en `REQUIRED_FILES`. | No mover sin actualizar `package.json`. |
| `scripts/dev/render-preview.js` | Preview rápido de una frase (`node scripts/dev/render-preview.js "texto" modo "#color"`), guarda PNG en `/output`. Es `npm run render`. | `package.json` (`render`), `CLAUDE.md`, `docs/architecture/mapa-del-proyecto.md`, `docs/operations/scripts.md`, `docs/operations/orden-para-ejecucion.md`, `.claude/settings.local.json` (config local no versionada, lo usa para pruebas rápidas). | render preview | Medio — no está en `REQUIRED_FILES`, pero sí es el checklist recomendado en `docs/architecture/mapa-del-proyecto.md` para validar cambios visuales retro3D. | No mover sin actualizar `package.json` y el checklist de cambios visuales. |
...
| `tools/archivo-x-curator.html` | Fallback activo servido por `archive-curator-server.js` (ruta catch-all) — UI legacy de curaduría ("Curaduría" + "Publicar carruseles"), alcanzable en producción vía el deploy de Render. | `CLAUDE.md`, `docs/architecture/entrypoints.md`, `docs/architecture/mapa-del-proyecto.md`, `docs/operations/scripts.md`, `doctor.js` (`REQUIRED_FILES`), `archive-curator-server.js` (`res.sendFile(...)`), `panel.html` (URL por defecto del curador apunta al mismo backend que lo sirve), `render.yaml` (nombre del servicio). | fallback activo | **Alto** — ya documentado en detalle en `docs/architecture/entrypoints.md`. No es HTML muerto. | No mover en esta fase (ya marcado explícitamente en el propio archivo y en `entrypoints.md`). |
...
- **curaduría activa / fallback activo:** `archive-curator-server.js` y `tools/archivo-x-curator.html` — los dos archivos "delicados" de esta carpeta, ya documentados aparte.
- **temporal/debug:** `debug-layout-tmp.js` — el único sin ninguna referencia oficial en README/CLAUDE, pero conservado a propósito.
- **candidato a legacy:** ninguno. Los 8 archivos de `scripts/dev/` y el único archivo de `tools/` están en uso o son diagnóstico activo — no encontré código muerto real en esta auditoría.

## Qué NO se debe mover todavía

- `scripts/dev/archive-curator-server.js` y `tools/archivo-x-curator.html` — por las razones ya
  documentadas en `docs/architecture/entrypoints.md` (producción real vía Render).
