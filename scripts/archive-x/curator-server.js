...
async function ensureArchiveContract(sheets, rows) {
  ...
  const isCleanContract =
    currentHeaders.length === REQUIRED_HEADERS.length &&
    REQUIRED_HEADERS.every((header, index) => currentHeaders[index] === header);

  if (isCleanContract) {
    return rows;
  }

  const legacyFound = currentHeaders.filter(header => LEGACY_COLUMNS.has(header));
  ...
  const cleanRows = [
    REQUIRED_HEADERS,
    ...rows.slice(1).map(row => REQUIRED_HEADERS.map(header => {
  ...
  await sheets.spreadsheets.values.update({
    spreadsheetId: SHEET_ID,
    range: `${WORKSHEET_NAME}!A1:${colToLetter(REQUIRED_HEADERS.length)}${cleanRows.length}`,
  ...
  if (properties?.sheetId !== undefined && columnCount > REQUIRED_HEADERS.length) {
        ...
        requests: [
          {
            deleteDimension: {
              range: {
                ...
                dimension: "COLUMNS",
                startIndex: REQUIRED_HEADERS.length,
...
async function loadArchive(sheets) {
  const rows = await ensureArchiveContract(sheets, await readSheetRows(sheets));
...
async function updateRow(sheets, rowNumber, patch) {
  let rows = await ensureArchiveContract(sheets, await readSheetRows(sheets));
  ...
  rows = await ensureArchiveContract(sheets, await readSheetRows(sheets));
